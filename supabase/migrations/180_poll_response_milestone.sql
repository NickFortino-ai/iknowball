-- Track when a poll has crossed the 6-non-author-voters threshold so we
-- only fire the milestone notification once per poll.
ALTER TABLE hot_takes ADD COLUMN IF NOT EXISTS voters_notified_at TIMESTAMPTZ;
