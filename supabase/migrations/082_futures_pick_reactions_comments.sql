...
ALTER TABLE feed_reactions ADD CONSTRAINT feed_reactions_target_type_check
  CHECK (target_type IN ('pick','parlay','streak_event','record_history','hot_take','head_to_head','futures_pick'));
...
ALTER TABLE comments ADD CONSTRAINT comments_target_type_check
  CHECK (target_type IN ('pick', 'parlay', 'prop', 'streak_event', 'record_history', 'hot_take', 'head_to_head', 'futures_pick'));
