...
const createLeagueSchema = z.object({
  ...
  format: z.enum(['pickem', 'survivor', 'squares', 'bracket', 'fantasy', 'nba_dfs', 'mlb_dfs', 'wnba_dfs', 'hr_derby', 'strikeouts', 'three_point', 'wnba_three_point', 'sacks', 'ints', 'tackles', 'receptions', 'td_pass']),
  sport: z.enum(['americanfootball_nfl', 'basketball_nba', 'baseball_mlb', 'basketball_ncaab', 'americanfootball_ncaaf', 'basketball_wnba', 'basketball_wncaab', 'icehockey_nhl', 'soccer_usa_mls', 'soccer_world_cup', 'americanfootball_ufl', 'all']),
  duration: z.enum(['this_week', 'custom_range', 'full_season']).optional(),
  ...
  fantasy_settings: z.object({
    ...
    trade_review: z.enum(['commissioner', 'none']).optional(),
...
router.get('/backdrops', requireAuth, async (req, res) => {
  ...
  // 'all' is the All-Sports pill (Survivor / Pick'em variant) — every backdrop
  // is fair game since the league spans every sport. Treat it like no filter.
...
router.post('/:id/fantasy/trades', requireAuth, async (req, res) => {
  try {
    const { receiver_user_id, proposer_player_ids, receiver_player_ids, message, counters_trade_id, proposer_drop_player_ids } = req.body
    const result = await proposeTrade(
      ...
      message,
      counters_trade_id,
      proposer_drop_player_ids || [],
    )
    res.json(result)
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message, requires_drop: err.requires_drop, drops_needed: err.drops_needed, side: err.side })
  }
})
