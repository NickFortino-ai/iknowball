...
router.get('/players', async (req, res) => {
  ...
  pool.sort((a, b) => {
    ...
    if (hasStats) {
      ...
      return b.season_sacks - a.season_sacks
        || (PRESEASON_SACK_RANK[a.player_name] ?? 999) - (PRESEASON_SACK_RANK[b.player_name] ?? 999)
...
// Defenders that are exhausted for this season under the league's
// pick_reuse setting (commissioner-tunable: 1x/2x/3x/4x/Unlimited).
// View grays these in the available list. Excludes the current week
// since today's picks are still replaceable.
