...
router.post('/picks', async (req, res) => {
  ...
  // wiping a row for a kept player and re-inserting a fresh row briefly resets
  // its accumulated stats (threes_made / points) to 0 until the next scoring
  // sync, which is a jarring UX flash for live games.
  ...
  if (toDeleteIds.length) {
    await supabase.from('three_point_picks').delete().in('id', toDeleteIds)
  }
  if (toInsert.length) {
    const { error: pickErr } = await supabase.from('three_point_picks').insert(toInsert)
