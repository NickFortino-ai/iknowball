  ...
  const { data: league, error } = await supabase
    ...
    .insert({
      ...
      joins_locked_at: data.joins_locked_at
        ? (['nba_dfs', 'mlb_dfs', 'hr_derby', 'strikeouts', 'three_point'].includes(data.format) && data.joins_locked_at.length === 10
...
export async function updateLeague(leagueId, userId, data) {
  ...
  if (!noteOnly && league.status !== 'open') {
    ...
    const ENDS_AT_EDITABLE_FORMATS = ['hr_derby', 'strikeouts', 'three_point', 'sacks', 'ints', 'nba_dfs', 'mlb_dfs', 'pickem']
