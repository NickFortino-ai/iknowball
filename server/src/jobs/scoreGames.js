...
import { scoreSurvivorPicks, scoreTouchdownSurvivorPicks } from '../services/survivorService.js'
...
async function scoreSport(sportKey) {
  ...
  for (const event of completedEvents) {
    ...
    try {
      ...
      await scoreTouchdownSurvivorPicks(game.id)
      await scoreLeaguePicks(game.id, winner)

      if (winner) {
        try {
          await scoreBracketMatchups(game.home_team, game.away_team, winner, homePoints, awayPoints, sportKey)
        } catch (err) {
    ...
    // Fetch top scorers from ESPN box score (fire-and-forget, don't block scoring)
    ;(async () => {
      try {
...
export async function scoreGames() {
  ...
  const sports = ['americanfootball_nfl', 'basketball_nba', 'baseball_mlb', 'basketball_ncaab', 'basketball_wncaab', 'americanfootball_ncaaf', 'basketball_wnba', 'icehockey_nhl', 'soccer_usa_mls', 'soccer_world_cup', 'americanfootball_ufl']
  let total = 0
