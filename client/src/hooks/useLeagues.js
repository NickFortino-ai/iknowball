...
export function useRespondToTrade(leagueId) {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ tradeId, action, drop_player_ids, reason }) => {
      const body = {}
      if (drop_player_ids?.length) body.drop_player_ids = drop_player_ids
      if (reason) body.reason = reason
      return api.post(`/leagues/${leagueId}/fantasy/trades/${tradeId}/${action}`, Object.keys(body).length ? body : undefined)
    },
    onSuccess: () => {
...
export function useBlurbPlayerIds(leagueId) {
  return useQuery({
    queryKey: ['fantasy', leagueId, 'blurb-ids'],
    queryFn: () => api.get(`/leagues/${leagueId}/fantasy/blurb-ids`),
    enabled: !!leagueId,
...
export function useIntsPicks(leagueId) {
  return useQuery({
    queryKey: ['ints', leagueId, 'picks'],
    queryFn: () => api.get(`/ints/picks?league_id=${leagueId}`),
    enabled: !!leagueId,
    refetchInterval: 2 * 60 * 1000,
  })
}

export function useIntsUsed(leagueId) {
  return useQuery({
    queryKey: ['ints', leagueId, 'used'],
    queryFn: () => api.get(`/ints/used?league_id=${leagueId}`),
    enabled: !!leagueId,
  })
}

export function useSubmitIntsPicks() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data) => api.post('/ints/picks', data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['ints', variables.league_id] })
    },
  })
}

export function useIntsStandings(leagueId) {
  return useQuery({
    queryKey: ['ints', leagueId, 'standings'],
    queryFn: () => api.get(`/ints/standings?league_id=${leagueId}`),
    enabled: !!leagueId,
    refetchInterval: 60000,
  })
}
