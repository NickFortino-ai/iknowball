...
export default function PickButton({ team, odds, score, isLive, state = 'default', onClick, disabled }) {
  ...
  return (
    ...
    >
      <div className={`font-semibold text-xs sm:text-sm mb-1 truncate ${state === 'correct' ? 'text-correct' : state === 'incorrect' ? 'text-incorrect' : state === 'locked-picked' ? 'text-white' : 'text-text-primary'}`}>
        {team}
      </div>
      {score != null ? (
        <div className={`text-lg font-display ${isLive ? 'text-accent' : state === 'correct' ? 'text-correct' : state === 'incorrect' ? 'text-incorrect' : state === 'locked-picked' ? 'text-accent' : 'text-text-primary'}`}>{score}</div>
      ) : (
