...
const sideStyles = {
  ...
  'locked-selected': 'bg-bg-primary border-accent/50 cursor-not-allowed',
...
export default function PropCard({ prop, pick, onPick, onUndoPick, isSubmitting, compact }) {
  ...
  return (
    <div className={`bg-bg-primary/50 backdrop-blur-sm rounded-2xl border ${pick?.status === 'locked' ? 'border-accent' : 'border-text-primary/20'} p-4`}>
      ...
      {pick?.live_stat != null && pick?.status === 'locked' && (
        <div className="mt-2 text-center">
          <span className="font-display text-lg text-white">{pick.live_stat}</span>
