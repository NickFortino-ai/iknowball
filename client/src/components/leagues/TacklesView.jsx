...
import { useTacklesPlayers, useTacklesPicks, useTacklesUsed, useSubmitTacklesPicks, useTacklesStandings } from '../../hooks/useLeagues'
...
function tackleLabel(n) {
  return Number(n) === 1 ? 'tackle' : 'tackles'
}
...
export default function TacklesView({ league, tab = 'picks' }) {
  const { profile } = useAuth()

  const { data: poolData, isLoading: playersLoading } = useTacklesPlayers()
  const { data: myPicks, isLoading: picksLoading } = useTacklesPicks(league.id)
  const { data: usedPlayers } = useTacklesUsed(league.id)
  const submitPicks = useSubmitTacklesPicks()
  const { data: standingsData } = useTacklesStandings(league.id)
  ...
  // Keep usage counts handy for the "Used N/N this season" label.
        ...
        {!standings.length ? (
          ...
          <div className="rounded-2xl border border-text-primary/15 bg-bg-primary/15 backdrop-blur-md overflow-hidden">
            <div className="grid grid-cols-[1.5rem_1fr_3rem] lg:grid-cols-[2rem_1fr_3.5rem] gap-1.5 lg:gap-3 px-3 lg:px-5 py-3 border-b border-text-primary/10 text-xs text-text-muted uppercase tracking-wider">
              <span>#</span>
              <span>Player</span>
              <span className="text-right">Tackles</span>
            </div>
            {standings.map((s) => {
              ...
              return (
                <div key={s.user?.id} className="border-b border-text-primary/10 last:border-b-0">
                  ...
                  >
                    ...
                    <span className="font-display text-lg lg:text-xl text-white text-right">{s.totalTackles}</span>
                  </div>
                  {isExpanded && (() => {
                        ...
                        <div>
                          ...
                          {!thisWeekPicks.length ? (
                            ...
                            <div className="space-y-1.5">
                              {thisWeekPicks.map((pick, i) => pick.hidden ? (
                                <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/5 border border-text-primary/10 border-dashed rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                                  ...
                                  <span className="font-display text-sm lg:text-base shrink-0 text-text-muted">—</span>
                                </div>
                              ) : (
                                <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                                  ...
                                  <span className={`font-display text-sm lg:text-base shrink-0 ${pick.tackles > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.tackles} {tackleLabel(pick.tackles)}</span>
                                </div>
                              ))}
                            </div>
                          )}
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      <div className="order-1 lg:col-start-1 lg:row-start-1">
        ...
        <div className="rounded-xl border border-text-primary/15 bg-bg-primary/10 backdrop-blur-sm p-4 mb-4">
          ...
          {selected.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-4">Pick 3 defenders who you think will rack up tackles this week</p>
          ) : (
            <div className="space-y-2">
              {selected.map((player) => {
                const savedPick = (myPicks || []).find((p) => p.sleeper_player_id === player.sleeper_player_id)
                const tackles = Number(savedPick?.tackles) || 0
                return (
                  <div
                  ...
                  >
                    ...
                    {hasSavedPicks && !editing && (
                      <span className={`font-display text-lg shrink-0 ${tackles > 0 ? 'text-correct' : 'text-text-muted'}`}>{tackles}</span>
                    )}
                    {(!hasSavedPicks || editing) && (
                      <button
        ...
        {hasSavedPicks && !editing && (() => {
          const weekTackles = (myPicks || []).reduce((sum, p) => sum + (Number(p.tackles) || 0), 0)
          return (
            <div className="flex items-center justify-end gap-4 pr-7 mb-2 -mt-1">
              ...
              <span className="font-display flex items-baseline gap-1">
                <span className={`text-lg ${weekTackles > 0 ? 'text-correct' : 'text-text-muted'}`}>{weekTackles}</span>
                <span className="text-sm text-white">{tackleLabel(weekTackles)}</span>
              </span>
            </div>
          )
        })()}
            ...
            {filteredPlayers.map((player) => {
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <span className="font-display text-base text-white whitespace-nowrap shrink-0">{player.season_tackles || 0}</span>
                </div>
                {(!hasSavedPicks || editing) && (
                  <button
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className={`font-display text-sm shrink-0 ${pick.tackles > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.tackles}</span>
                      </div>
                    ))}
                  </div>
                </div>
