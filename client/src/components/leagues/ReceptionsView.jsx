...
export default function ReceptionsView({ league, tab = 'picks' }) {
  ...
  const { data: poolData, isLoading: playersLoading } = useReceptionsPlayers()
  const { data: myPicks, isLoading: picksLoading } = useReceptionsPicks(league.id)
  const { data: usedPlayers } = useReceptionsUsed(league.id)
  const submitPicks = useSubmitReceptionsPicks()
  const { data: standingsData } = useReceptionsStandings(league.id)
              ...
              return (
                <div key={s.user?.id} className="border-b border-text-primary/10 last:border-b-0">
                  ...
                  >
                    ...
                    <span className="font-display text-lg lg:text-xl text-white text-right">{s.totalReceptions}</span>
                  </div>
                  {isExpanded && (() => {
                            ...
                            <div className="space-y-1.5">
                              {thisWeekPicks.map((pick, i) => pick.hidden ? (
                                ...
                                <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                                  ...
                                  <span className={`font-display text-sm lg:text-base shrink-0 ${pick.receptions > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.receptions} {receptionLabel(pick.receptions)}</span>
                                </div>
                              ))}
                            </div>
                          )}
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      <div className="order-1 lg:col-start-1 lg:row-start-1">
        ...
        <div className="rounded-xl border border-text-primary/15 bg-bg-primary/10 backdrop-blur-sm p-4 mb-4">
          ...
          {selected.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-4">Pick up to 3 pass catchers who you think will rack up receptions this week</p>
          ) : (
            <div className="space-y-2">
              {selected.map((player) => {
                const savedPick = (myPicks || []).find((p) => p.sleeper_player_id === player.sleeper_player_id)
                const receptions = Number(savedPick?.receptions) || 0
                return (
                  <div
                  ...
                  >
                    ...
                    {hasSavedPicks && !editing && (
                      <span className={`font-display text-lg shrink-0 ${receptions > 0 ? 'text-correct' : 'text-text-muted'}`}>{receptions}</span>
                    )}
                    {(!hasSavedPicks || editing) && (
                      <button
        ...
        {hasSavedPicks && !editing && (() => {
          const weekReceptions = (myPicks || []).reduce((sum, p) => sum + (Number(p.receptions) || 0), 0)
          return (
            <div className="flex items-center justify-end gap-4 pr-7 mb-2 -mt-1">
              ...
              <span className="font-display flex items-baseline gap-1">
                <span className={`text-lg ${weekReceptions > 0 ? 'text-correct' : 'text-text-muted'}`}>{weekReceptions}</span>
                <span className="text-sm text-white">{receptionLabel(weekReceptions)}</span>
              </span>
            </div>
          )
        })()}
      ...
      <div className="order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2 rounded-xl border border-text-primary/15 bg-bg-primary/15 backdrop-blur-md overflow-hidden lg:max-h-[calc(100vh-200px)] lg:overflow-y-auto lg:sticky lg:top-4">
        <div className="px-4 py-3 border-b border-text-primary/10">
          <h3 className="text-sm font-semibold text-text-primary mb-3">Available Pass Catchers</h3>
          <input
            type="text"
            value={search}
        ...
        {filteredPlayers.length > 0 && (
          <div className="flex items-center justify-end px-4 py-2 border-b border-text-primary/10">
            <span className="text-xs font-semibold text-text-primary">Total Recs</span>
          </div>
        )}

        {!filteredPlayers.length ? (
          <div className="px-4 py-6 text-center text-xs text-text-muted">
            {!players?.length ? 'No pass catchers available yet — player data syncs after games are played.' : 'No players match your search.'}
          </div>
        ) : (
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className={`font-display text-sm shrink-0 ${pick.receptions > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.receptions}</span>
                      </div>
                    ))}
                  </div>
                </div>
