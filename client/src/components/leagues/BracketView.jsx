...
export default function BracketView({ league, tab = 'bracket', onTabChange, tabs: heroTabs, activeTabIndex, onTabSelect, threadUnread }) {
  ...
  return (
    <div className="relative z-10 overflow-x-hidden lg:overflow-x-visible">
      {/* Hero area */}
      <div className="relative rounded-xl mb-4 overflow-hidden">
        ...
        <div className="relative z-10 p-4">
          ...
          {isLocked && !hasSubmitted && !entryLoading && (
            <div className="bg-bg-primary/40 backdrop-blur-sm border border-text-primary/20 rounded-xl p-6 text-center">
              <div className="text-lg text-text-primary font-bold mb-3">Bracket not submitted in time</div>
              ...
              {heroTabs.map((t, i) => (
                <button
                  ...
                  className={`relative px-4 py-2 rounded-lg text-sm font-semibold transition-colors border backdrop-blur-sm ${
                    activeTabIndex === i
                      ? 'bg-bg-primary/30 border-accent text-accent'
                      : 'bg-bg-primary/30 border-text-primary/20 text-text-secondary hover:border-text-primary/40'
                  }`}
                >
                  {t}
              ...
              <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                <button
                  ...
                  className={`shrink-0 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                    !viewingUserId ? 'bg-bg-primary/30 border border-accent text-accent' : 'bg-bg-primary/30 border border-text-primary/20 text-text-primary hover:border-text-primary/40'
                  }`}
                >
                  Master
                </button>
                {entries.map((e) => (
                  <button
                    ...
                    className={`shrink-0 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                      viewingUserId === e.user_id ? 'bg-bg-primary/30 border border-accent text-accent' : 'bg-bg-primary/30 border border-text-primary/20 text-text-primary hover:border-text-primary/40'
                    }`}
                  >
