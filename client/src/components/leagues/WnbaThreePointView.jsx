...
import { useWnbaThreePointPlayers, useWnbaThreePointPicks, useWnbaThreePointUsed, useSubmitWnbaThreePointPicks, useWnbaThreePointStandings } from '../../hooks/useLeagues'
...
export default function WnbaThreePointView({ league, tab = 'picks' }) {
  ...
  const { data: players, isLoading: playersLoading } = useWnbaThreePointPlayers(date)
  const { data: myPicks, isLoading: picksLoading } = useWnbaThreePointPicks(league.id, date)
  const { data: usedPlayers } = useWnbaThreePointUsed(league.id, date)
  const submitPicks = useSubmitWnbaThreePointPicks()
  const { data: standingsData } = useWnbaThreePointStandings(league.id)
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      ...
      {detailPlayer && (
        <PlayerDetailModal player={detailPlayer} onClose={() => setDetailPlayer(null)} sport="basketball_wnba" />
      )}
    </div>
  )
}
