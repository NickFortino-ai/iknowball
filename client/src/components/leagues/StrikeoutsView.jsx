...
import { useStrikeoutsPlayers, useStrikeoutsPicks, useStrikeoutsUsed, useSubmitStrikeoutsPicks, useStrikeoutsStandings } from '../../hooks/useLeagues'
...
export default function StrikeoutsView({ league, tab = 'picks' }) {
  ...
  const { data: players, isLoading: playersLoading } = useStrikeoutsPlayers(date)
  const { data: myPicks, isLoading: picksLoading } = useStrikeoutsPicks(league.id, date)
  const { data: usedPlayers } = useStrikeoutsUsed(league.id, date)
  const submitPicks = useSubmitStrikeoutsPicks()
  const { data: standingsData } = useStrikeoutsStandings(league.id)
              ...
              return (
                <div key={s.user?.id} className="border-b border-text-primary/10 last:border-b-0">
                  ...
                  >
                    ...
                    <span className="font-display text-lg lg:text-xl text-white text-right">{s.totalStrikeouts}</span>
                  </button>
                  {isExpanded && (() => {
                        ...
                        <div className="space-y-1.5">
                          {todayPicks.map((pick, i) => (
                            <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/20 border border-text-primary/10 rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                              ...
                              <span className={`font-display text-sm lg:text-base shrink-0 ${pick.strikeouts > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.strikeouts} K</span>
                            </div>
                          ))}
                        </div>
                      )}
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      ...
      <div className="order-1 lg:col-start-1 lg:row-start-1">
        ...
        <div className="rounded-xl border border-text-primary/15 bg-bg-primary/10 backdrop-blur-sm p-4 mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-text-primary">Today's Strikeout Picks</h3>
            <span className="text-xs text-text-muted">{selected.length}/3 picks</span>
          </div>

          {selected.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-4">Pick up to 3 pitchers you think will rack up strikeouts today</p>
          ) : (
            <div className="space-y-2">
              {selected.map((player) => {
                ...
                const ks = savedPick?.strikeouts || 0
        ...
        {hasSavedPicks && !editing && (() => {
          const dayKs = (myPicks || []).reduce((sum, p) => sum + (p.strikeouts || 0), 0)
          return (
            <div className="flex items-center justify-end gap-4 px-1 mb-2 -mt-1">
              ...
              <span className={`font-display text-sm ${dayKs > 0 ? 'text-correct' : 'text-text-muted'}`}>{dayKs} K{dayKs === 1 ? '' : 's'}</span>
            </div>
          )
        })()}
      ...
      <div className="order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2 rounded-xl border border-text-primary/15 bg-bg-primary/30 backdrop-blur-md overflow-hidden lg:max-h-[calc(100vh-200px)] lg:overflow-y-auto lg:sticky lg:top-4">
        <div className="px-4 py-3 border-b border-text-primary/10">
          <h3 className="text-sm font-semibold text-text-primary mb-3">Available Pitchers</h3>
          <input
            type="text"
            value={search}
            ...
            {filteredPlayers.map((player) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <span className="font-display text-base text-white whitespace-nowrap shrink-0">{player.season_strikeouts || 0} K</span>
                </div>
                {(!hasSavedPicks || editing) && (
                  <button
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/20 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className={`font-display text-sm shrink-0 ${pick.strikeouts > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.strikeouts} K{pick.strikeouts === 1 ? '' : 's'}</span>
                      </div>
                    ))}
                  </div>
                </div>
