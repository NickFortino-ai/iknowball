    ...
    <div className="space-y-2">
      {members?.map((m) => {
        ...
        return (
          <div
            key={m.id}
            className={`bg-bg-primary rounded-xl border border-text-primary/20 px-4 py-3 flex items-center justify-between${onUserTap ? ' cursor-pointer hover:bg-bg-primary/80 transition-colors' : ''}`}
        ...
        <>
          ...
          {filteredPending.map((inv) => {
            ...
            return (
              <div
                key={inv.id}
                className="bg-bg-primary rounded-xl border border-text-primary/20 px-4 py-3 flex items-center justify-between opacity-60"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <Avatar user={user} size="xl" />
                  <div className="min-w-0">
