...
export default function ThreePointView({ league, tab = 'picks' }) {
                            ...
                            {todayPicks.map((pick, i) => pick.hidden ? (
                              <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/5 border border-text-primary/10 border-dashed rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                                ...
                                <span className="font-display shrink-0 flex items-baseline gap-1">
                                  ...
                                  <span className="text-xs lg:text-sm text-white">3PM</span>
                                </span>
                              </div>
                            ) : (
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
                ...
                return (
                  ...
                  >
                    ...
                    {(hasSavedPicks && !editing) || (editing && isPickLocked(savedPick)) ? (
                      <span className="font-display shrink-0 flex items-baseline gap-1">
                        <span className={`text-lg ${threes > 0 ? 'text-correct' : 'text-text-muted'}`}>{threes}</span>
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/20 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className="font-display shrink-0 flex items-baseline gap-1">
                          <span className={`text-sm ${pick.made_threes > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.made_threes}</span>
      ...
      {detailPlayer && (
        <PlayerDetailModal player={detailPlayer} onClose={() => setDetailPlayer(null)} sport="basketball_nba" />
      )}
    </div>
  )
}
