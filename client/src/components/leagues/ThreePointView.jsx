...
import { useThreePointPlayers, useThreePointPicks, useThreePointUsed, useSubmitThreePointPicks, useThreePointStandings } from '../../hooks/useLeagues'
...
export default function ThreePointView({ league, tab = 'picks' }) {
  ...
  const { data: players, isLoading: playersLoading } = useThreePointPlayers(date)
  const { data: myPicks, isLoading: picksLoading } = useThreePointPicks(league.id, date)
  const { data: usedPlayers } = useThreePointUsed(league.id, date)
  const submitPicks = useSubmitThreePointPicks()
  const { data: standingsData } = useThreePointStandings(league.id)
  ...
  // A pick is locked once its game tips off (live or final).
  const now = new Date()
  ...
  function addPlayer(player) {
    if (selected.length >= 3) {
      toast('Maximum 3 picks per night', 'error')
      return
    }
        ...
        {!standings.length ? (
          ...
          <div className="rounded-2xl border border-text-primary/15 bg-bg-primary/30 backdrop-blur-md overflow-hidden">
            <div className="grid grid-cols-[1.5rem_1fr_3rem] lg:grid-cols-[2rem_1fr_3.5rem] gap-1.5 lg:gap-3 px-3 lg:px-5 py-3 border-b border-text-primary/10 text-xs text-text-muted uppercase tracking-wider">
              <span>#</span>
              <span>Player</span>
              <span className="text-right">3PM</span>
            </div>
            {standings.map((s) => {
                ...
                <div key={s.user?.id} className="border-b border-text-primary/10 last:border-b-0">
                  ...
                  {isExpanded && (() => {
                    ...
                    <div className="px-3 lg:px-5 pb-3">
                      {!todayPicks.length ? (
                        <p className="text-xs text-text-muted text-center py-2">No picks tonight</p>
                      ) : (
                        <div className="space-y-1.5">
                          {todayPicks.map((pick, i) => (
                            <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/20 border border-text-primary/10 rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                              ...
                              <span className={`font-display text-sm lg:text-base shrink-0 ${pick.made_threes > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.made_threes} 3PM</span>
                            </div>
                          ))}
                        </div>
                      )}
    ...
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      <div className="order-1 lg:col-start-1 lg:row-start-1">
        ...
        <div className="rounded-xl border border-text-primary/15 bg-bg-primary/10 backdrop-blur-sm p-4 mb-4">
          ...
          {selected.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-4">Pick up to 3 shooters who you think will hit threes tonight</p>
          ) : (
            <div className="space-y-2">
              {selected.map((player) => {
                const savedPick = (myPicks || []).find((p) => p.espn_player_id === player.espn_player_id)
                const threes = savedPick?.made_threes || 0
                ...
                return (
                  <div key={player.espn_player_id} className="flex items-center gap-2 bg-bg-primary/10 border border-text-primary/15 rounded-lg px-3 py-2.5">
                    ...
                    {(hasSavedPicks && !editing) || (editing && isPickLocked(savedPick)) ? (
                      <span className={`font-display text-lg shrink-0 ${threes > 0 ? 'text-correct' : 'text-text-muted'}`}>{threes} 3PM</span>
                    ) : null}
        ...
        {hasSavedPicks && !editing && (() => {
          ...
          return (
            <div className="flex items-center justify-end gap-4 px-1 mb-2 -mt-1">
              ...
              <span className={`font-display text-sm ${dayThrees > 0 ? 'text-correct' : 'text-text-muted'}`}>{dayThrees} 3PM</span>
            </div>
          )
        })()}
        ...
        {hasSavedPicks && !editing && !allPicksLocked ? (
          ...
          <div className="w-full py-3 rounded-xl text-center text-sm text-text-muted bg-bg-primary/30 border border-text-primary/10 mb-6">
            All games are final — picks locked for the night
          </div>
        ) : (
          <button
      ...
      <div className="order-2 lg:col-start-2 lg:row-start-1 lg:row-span-2 rounded-xl border border-text-primary/15 bg-bg-primary/30 backdrop-blur-md overflow-hidden lg:max-h-[calc(100vh-200px)] lg:overflow-y-auto lg:sticky lg:top-4">
        <div className="px-4 py-3 border-b border-text-primary/10">
          <h3 className="text-sm font-semibold text-text-primary mb-3">Available Shooters</h3>
          <input
            type="text"
            value={search}
        ...
        {filteredPlayers.length > 0 && (
          <div className="flex items-center justify-end px-4 py-2 border-b border-text-primary/10">
            <span className="text-xs font-semibold text-text-primary">Total 3PM</span>
          </div>
        )}
        ...
        {!filteredPlayers.length ? (
          ...
          <div className="max-h-[50vh] overflow-y-auto">
            {filteredPlayers.map((player) => {
              // Used in another period this week — keep visible but
              // grayed out with the add button disabled, so the user
              // sees who they've burned without scrolling a separate
              // 'Used This Week' chip row.
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/20 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className={`font-display text-sm shrink-0 ${pick.made_threes > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.made_threes} 3PM</span>
                      </div>
                    ))}
                  </div>
                </div>
