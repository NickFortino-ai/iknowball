...
import { useIntsPlayers, useIntsPicks, useIntsUsed, useSubmitIntsPicks, useIntsStandings } from '../../hooks/useLeagues'
...
export default function IntsView({ league, tab = 'picks' }) {
  const { profile } = useAuth()

  const { data: poolData, isLoading: playersLoading } = useIntsPlayers()
  const { data: myPicks, isLoading: picksLoading } = useIntsPicks(league.id)
  const { data: usedPlayers } = useIntsUsed(league.id)
  const submitPicks = useSubmitIntsPicks()
  const { data: standingsData } = useIntsStandings(league.id)
        ...
        {!standings.length ? (
          ...
          <div className="rounded-2xl border border-text-primary/15 bg-bg-primary/15 backdrop-blur-md overflow-hidden">
            <div className="grid grid-cols-[1.5rem_1fr_3rem] lg:grid-cols-[2rem_1fr_3.5rem] gap-1.5 lg:gap-3 px-3 lg:px-5 py-3 border-b border-text-primary/10 text-xs text-text-muted uppercase tracking-wider">
              <span>#</span>
              <span>Player</span>
              <span className="text-right">INTs</span>
            </div>
            {standings.map((s) => {
              ...
              return (
                <div key={s.user?.id} className="border-b border-text-primary/10 last:border-b-0">
                  ...
                  >
                    ...
                    <span className="font-display text-lg lg:text-xl text-white text-right">{s.totalInts}</span>
                  </div>
                  {isExpanded && (() => {
                            ...
                            <div className="space-y-1.5">
                              {thisWeekPicks.map((pick, i) => pick.hidden ? (
                                ...
                                <div key={i} className="flex items-center gap-2 lg:gap-3 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 lg:px-4 py-2 lg:py-3">
                                  ...
                                  <span className={`font-display text-sm lg:text-base shrink-0 ${pick.ints > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.ints} INT{pick.ints === 1 ? '' : 's'}</span>
                                </div>
                              ))}
                            </div>
                          )}
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 pb-24 lg:pb-0">
      <div className="order-1 lg:col-start-1 lg:row-start-1">
                ...
                return (
                  ...
                  >
                    ...
                    {hasSavedPicks && !editing && (
                      <span className={`font-display text-lg shrink-0 ${ints > 0 ? 'text-correct' : 'text-text-muted'}`}>{ints}</span>
                    )}
                    {(!hasSavedPicks || editing) && (
                      <button
        ...
        {hasSavedPicks && !editing && (() => {
          const weekInts = (myPicks || []).reduce((sum, p) => sum + (Number(p.ints) || 0), 0)
          return (
            <div className="flex items-center justify-end gap-4 pr-7 mb-2 -mt-1">
              ...
              <span className="font-display flex items-baseline gap-1">
                <span className={`text-lg ${weekInts > 0 ? 'text-correct' : 'text-text-muted'}`}>{weekInts}</span>
                <span className="text-sm text-white">INT{weekInts === 1 ? '' : 's'}</span>
              </span>
            </div>
          )
        })()}
                  ...
                  <div className="space-y-1.5">
                    {picks.map((pick, i) => (
                      <div key={i} className="flex items-center gap-2 bg-bg-primary/10 border border-text-primary/10 rounded-lg px-2.5 py-2">
                        ...
                        <span className={`font-display text-sm shrink-0 ${pick.ints > 0 ? 'text-correct' : 'text-text-muted'}`}>{pick.ints}</span>
                      </div>
                    ))}
                  </div>
                </div>
